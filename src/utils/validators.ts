export function login (value: string, submit: boolean): string {
    ...
    if (!/^[a-z0-9_-]+$/.test(value)) return 'Логин может содержать только латинские символы, цифры, дефис и нижнее подчеркивание';
    return '';
}
...
export function phone (value: string, submit?: boolean): string {
    ...
    if (!/^[+]?[0-9]+$/.test(value)) return 'Телефон должен состоять только из цифр и может начинаться с плюса';
    return '';
}
...
export function email (value: string, submit?: boolean): string {
    if (value.length == 0 && !submit) return '';
    if (value.length == 0 && submit) return 'Поле не должно быть пустым';
    if (!/^[a-zA-Z0-9_-]+@[a-zA-Z0-9_-]+\.[a-zA-Z]{2,}/.test(value)) return 'E-mail заполнен неверно';
    return '';
}
...
export function name (value: string, submit?: boolean): string {
    if (value.length == 0 && !submit) return '';
    if (value.length == 0 && submit) return 'Поле не должно быть пустым';
    if (!/^[A-ZА-ЯЁ][a-zа-яё-]+$/.test(value)) return 'Начинать нужно с заглавной буквы, из спецсимволов допустим только дефис';
    return '';
}
